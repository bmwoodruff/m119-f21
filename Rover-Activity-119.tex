...
\begin{center}
  \begin{tikzpicture}[scale=1.91]
    ...
    \draw[->,thick] (0,0) -- (8,0)node[below]{$x$ (8000 ft)};
    \draw[->,thick] (0,0) -- (0,6)node[above]{$y$ (6000 ft)};
    \node at (-.15,1) {1};
    \node at (-.15,2) {2};
    \node at (-.15,3) {3};
    \node at (-.15,4) {4};
...
\end{tikzpicture}
\end{center}
The contours on the map represent a function $z=f(x,y)$, where $z$ is the elevation in feet, with contours drawn at 20 ft increments.  Each grid line represents 1000 ft. Suppose a hiker is currently at $A$. The hiker wants to get to $B$. Discuss the answers to each question below with your group.
...
\begin{enumerate}
 ...
 \begin{enumerate}
  \item At point $C$, notice that moving right $\Delta x \approx 100$ ft (so .1 of 1000 ft) doesn't really change the elevation much at all (maybe there is a slight drop, but very small). This suggests that $\Delta z\approx 0$ and so the slope when moving east is $\ds\dfrac{\Delta z}{\Delta x}\approx\frac{0}{100}=0$. 
  \item At point $C$, moving north about $\Delta y = 100$ ft gets us to the next contour, which is a drop of $\Delta z \approx -20$ ft.  This gives a slope of $\ds\dfrac{\Delta z}{\Delta y} \approx \frac{-20}{100} = -0.2$, a 20\% downhill grade. 
  \item Above we estimated $\ds\dfrac{\Delta z}{\Delta x}\approx\frac{0}{100}=0$ and $\ds\dfrac{\Delta z}{\Delta y} \approx \frac{-20}{100} = -0.2$. We can draw an arrow starting from point $C$ that moves right 0 units and up $-0.2$ units (so the arrow points straight south). This vector we call the gradient of $f$ at $C$ and write it symbolically as $$\vec \nabla f(C) = \left<0,-0.2\right> = 0{\bf i}-0.2{\bf j} = 0\vec i-0.2\vec j.$$
...
At each point $(x,y)$ in the plane, the partial derivatives produced the vector $\vec \nabla f(x,y)$. 
...
\end{itemize}
We'll go through the first portion of the notebook as a class, explore several different examples, and make some conjectures. 
